...
func reconstructTree(data map[uint64]internal.ClickhouseField, root *internal.FlameGraphNode, minValue uint64) {
	for _, i := range root.ChildrenIds {
		if data[i].Value > minValue {
			node := &internal.FlameGraphNode{
				Id:          data[i].Id,
...
func timeHandler(w http.ResponseWriter, req *http.Request) {
	...
	idQuery := strconv.FormatUint(internal.RootElementId, 10)
...
func getHandler(w http.ResponseWriter, req *http.Request) {
	...
	data := make(map[uint64]internal.ClickhouseField)
	for rows.Next() {
		var res internal.ClickhouseField
	...
	flameGraphTreeRoot := &internal.FlameGraphNode{
		Id:          data[internal.RootElementId].Id,
		Cluster:     data[internal.RootElementId].Cluster,
		Name:        data[internal.RootElementId].Name,
		Value:       data[internal.RootElementId].Value,
		Total:       data[internal.RootElementId].Total,
		Parent:      nil,
		ChildrenIds: data[internal.RootElementId].ChildrenIds,
	}

	if column == "mtime" {
		flameGraphTreeRoot.Total = data[internal.RootElementId].Value
	}
